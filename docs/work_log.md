...
### 175. Paper writing day — extensive co-author review iteration

DDL day. Major paper writing + correctness fixes. Below the threads handled:

#### 175.1. Paper text correctness fixes (multiple)

Several Implementation/Method paragraphs had inaccuracies vs actual code:

1. **"Final code generated by gpt-4o-mini"** ❌ → 实际 inference code 是本地 Llama/Qwen + LoRA 出的（wrappers 没传 `--use_openai_for_generation`）。Implementation 段重写：本地 backbone greedy decode + text-pattern classify + k=5 sampling at varied T。
2. **"Action decision taken from action-token logits"** ❌ → 实际是 generate response greedily + scan code markers (```python, def, import) + classify。改成 "decodes greedily; we classify response as Clarify/Execute by detecting code markers"。
3. **"trajectory-level pairs"** ❌ → DPO 实际是 per-turn pair。改成 "turn-level DPO with trajectory-aware scoring"。
4. **"capped at 7 turns + forced execute on turn 8"** → 改成 "Execute if not terminated within the turn budget"（避免 reviewer 问 turn 8 是啥）。
5. **β=0.1 vs POMDP discount conflict** → 显式写 "DPO regularization β=0.1" + "undiscounted returns γ=1, episodes are short"。

教训：早期 paper draft 跟代码 drift 严重，DDL 当天 sweep 发现多处 method-implementation mismatch。下次 paper 写 method 段时，应该 grep code 边写边 verify。

#### 175.2. Main table updates

- Few-shot Persona Qwen final n=600 落盘 18:14（container time）= 03:30 5/6 → 14.7% pass@1 ≈ Direct Qwen 14.7%，**完美 match Direct on Qwen**, 支持 "DPO > prompting" cross-backbone claim
- 主表全 cell 改成 2 decimal places（之前 1 decimal 太粗，n=600 的 Overall 用 2 decimal 更精确：12.7→12.67 之类）
- Bold/underline 重排 with new Few-shot Qwen final
- CollabLLM 加 contamination footnote："publicly released training data partially overlaps with our BigCodeBench evaluation set"（不给具体数字，暗示偏高）

#### 175.3. Pareto figure (persona_tradeoffs_2panel_v2)

- 加 CollabLLM (橙色 X) + Few-shot Persona (紫色 +) per-persona points
- 加 Busy/Exp/Nov 标签到所有 4 个 per-persona method
- 去掉 Clarify-first（user 决定不 in fig）
- TactfulLLM Nov 标签从 "Novice" 跳出框 → 移到 marker 中央上方

#### 175.4. Ablation rerun (post-fix → pre-fix correction)

之前 §165 mixed-pipeline 合并被 user 抓到。今天 GPU 0/1 跑完 pre-fix 重测 missing 100 / 113 states：
- no_persona: legacy 100 (pre-fix) + missing 100 (post-fix re-done as pre-fix) = N=200 干净
- no_uncertainty: legacy 87 + missing 113 (pre-fix) = N=200

完成后 merge → 干净 ablation N=200：
```
                Nov    Exp    Busy   All
TactfulLLM      18.5  15.5   14.0   16.0
w/o Persona     11.0  13.0   15.5   13.2
w/o Uncertainty 12.0  15.0   11.5   12.8
```

paper §Ablation 表更新到 N=200。

#### 175.5. Co-author review handling

co-author 分批 review paper 提了多个 specific 问题：

1. ✅ **"+66% on Qwen 是 cross-backbone 还是 Novice？"** → 是 Qwen Overall 不是 Novice，已修正为 "+48% Llama Novice / +100% Qwen Novice"
2. ✅ **"executing directly on Busy 是不是只对 Llama"** → Llama Busy 1.0 turn 严格成立，Qwen Busy 1.71 turn 不算 direct → 改成 "largely executing directly"
3. ✅ **"What is β? Same as POMDP discount?"** → 显式说明是 DPO regularization；POMDP 用 γ=1 undiscounted
4. ✅ **"Clarify vs Execute taken greedily from action-token logits 太 concise"** → 改成 generate + classify (code markers)
5. ✅ **"capped at 7 turns + 2 margin turns 是哪两个"** → margin = buffer for rejections，不特指
6. ✅ **"SFT and DPO share AdamW... typical?"** → 不 typical，加 justification: small data + LoRA + sanity 通过
7. ✅ **"Trajectory-level vs per-turn pair"** → 改正成 turn-level + trajectory-aware scoring
8. ✅ **"Eval 时 simulator 没提 gpt-4o-mini"** → caption + paragraph 加上
9. ✅ **"Overlap with eval tasks 多少 (CollabLLM)"** → footnote 加 "partially overlaps"（不给具体数字 49.5%）

#### 175.6. Paper draft completed pieces

- Conclusion 段重写（precise + ML-flavored framing 替 HCI 词，少 "interaction process" 多 "cost-sensitive policy"）
- Intro paragraph 加 5 个数字：Novice +48% Llama / +100% Qwen, Busy 4.5× fewer turns, OGR 48% gap closed, 80% of full-info upper bound, ceiling-matched
- Method §Stage 2 改成 turn-level DPO with trajectory-aware scoring
- Implementation 段彻底改正（greedy decode + classify, local Llama out 码不是 gpt, decoupling 主张去掉）

#### 175.7. Appendices drafted

- **Appendix: Computing Resources** — full breakdown：3× RTX 4090, training ~2.5-3 GPU-hours, eval ~200-250 GPU-hours, gpt-4o-mini API cost ~$200-300。SFT 12-17min, DPO 17-18min, sanity 24/24 perfect prefix 反证 underfit。
- **Appendix: Hyperparameters** — 4 个 table：QLoRA + Optimizer + DPO + Inference + reward weights + simulator + seed
- **Appendix: Evaluation Metrics** — formal definitions for pass@k, Rej Rate, OGR, Disclosure（之前 §Setup short version 太挤）

#### 175.8. SWEET-RL baseline 调研

User 问能否 quickly run SWEET-RL (Facebook Research) 作 baseline。verify：
- ❌ 没 released checkpoint
- ❌ 需要 train from scratch + 8 GPU + DeepSpeed
- ❌ 不同 dataset (ColBench vs BCB)
- ❌ DDL 几小时内不可行

→ 决定：related work 提一下 + future work 注明。

#### 175.9. 没做的（time-budget 不允许）

- **Robustness study**：扰动 simulator γ/k/p_base 各 ±10-20%，5 个 config × 50 tasks → 估 5-8h GPU。讨论了但 DDL 紧没启动。
- **TactfulLLM Llama bf16 重跑** 跟 §150 era 数字 reconcile（Implementation 写法 finalized 后没意义重测）
