...
def build_smart_query(state: dict, conversation: list) -> str:
    """v3: disclosed_items + FIRST answered Clarify turn user_reaction (limit 400 chars).

    Avoids v2 bloat (all turns) by including only first answered turn's free-text
    response (which usually contains the most informative gpt-4o context spec
    before pleasantries/repetition).
    """
    query = state.get("query", "")
    seen = set()
    hints = []

    # 1. Structured disclosed_items (clean oracle ground truth)
    ...
    # 2. First answered Clarify turn's user_reaction (limit 400 chars)
    for t in conversation:
        if t.get("action") == "Clarify" and t.get("answered_clarification", 0) > 0:
            ur = t.get("user_reaction", "")
            if isinstance(ur, str) and ur.strip() and ur.strip() != "Continue.":
                ur_clean = ur.strip()[:400]
                if ur_clean not in seen:
                    seen.add(ur_clean)
                    hints.append(f"[User context]: {ur_clean}")
                break  # ONLY first answered turn
