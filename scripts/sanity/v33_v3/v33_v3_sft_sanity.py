"""v33 v3 SFT sanity (stronger params)."""
...
LORA_DIR = str(PROJECT_ROOT / "models/v33_v3_sft")
TEST_PATH = PROJECT_ROOT / "data/seeds/test_states_v29_eval_50.jsonl"
N_STATES = 8
OUT_PATH = Path("/tmp/v33_v3_sft_sanity.json")
...
def main():
    ...
    for si, state in enumerate(states):
        for persona in PERSONAS:
            ...
            row = {
                ...
                "is_pure_clarify": ("?" in text200 and not (("```python" in text200) or ("\nimport " in text200))),
            }
            rows.append(row)
            tag = "Clarify" if row["starts_with_Clarify"] else (
                  "Execute" if row["starts_with_Execute"] else "OTHER")
            mark = "✅" if row["is_pure_clarify"] else ""
            print(f"[{si+1}/8] {persona.name:22s} prefix={tag:8s}  q={row['has_question_mark']}  code={row['has_code_marker']}  {mark}  text30={text30[:55]!r}", flush=True)

    n = len(rows)
    cp = sum(1 for r in rows if r["starts_with_Clarify"])
    pc = sum(1 for r in rows if r["is_pure_clarify"])
    print()
    print("=" * 72)
    print(f"TOTAL: {n}")
    print(f"  starts with 'Clarify\\n':  {cp}/{n} ({100*cp/n:.1f}%)")
    print(f"  Pure Clarify (q+no code): {pc}/{n} ({100*pc/n:.1f}%)")
    print("=" * 72)
    print()
    for p_name in ('Novice-Learner', 'Busy-Developer', 'Experienced-Engineer'):
        ...
        pc_p = sum(1 for r in sub if r['is_pure_clarify'])
        q = sum(1 for r in sub if r['has_question_mark'])
        code = sum(1 for r in sub if r['has_code_marker'])
        print(f"  {p_name:22s}: Clarify_pfx={cp_p}/{len(sub)}  Pure_Clarify={pc_p}/{len(sub)}  q={q}/{len(sub)}  code={code}/{len(sub)}")
