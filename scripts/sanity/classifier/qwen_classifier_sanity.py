"""Sanity check: compare v1 (30-token prefix) vs v2 (200-token full-text) action
classifier on Qwen2.5-7B-Instruct + v29 LoRA, on real eval state prompts.

Goal: count v1 vs v2 disagreements on the same greedy generation, and inspect
the actual generated text in disagreement cases to validate v2 rules.
"""
...
from policy.infer import build_action_selection_chat_prompt, _pick_action_v1, _pick_action_v2  # noqa: E402
...
LORA_DIR = str(PROJECT_ROOT / "models/v29_qwen_100states")
TEST_PATH = PROJECT_ROOT / "data/seeds/test_states_v29_eval_50.jsonl"
N_STATES = 8  # 8 × 3 persona = 24 examples
OUT_PATH = Path("/tmp/qwen_classifier_sanity.json")
...
def main():
    ...
    for si, state in enumerate(states):
        for persona in PERSONAS:
            ...
            row = {
                ...
                "n_masked": len(state.get("disclosure_rule", {}).get("masked_fields", {}) or {}),
                "text30": text30,
                "text200": text200,
                "v1": v1_verdict,
                "v2": v2_verdict,
    ...
    print("=" * 72)
    print()
    print("Disagreement cases (v1 missed code that v2 caught):")
    for r in rows:
        if r["v1"] == "Clarify" and r["v2"] == "Execute":
            print(f"  state={r['state_id']} persona={r['persona']}")
